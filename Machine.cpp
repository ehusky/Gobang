...
COORDINATE MACHINE::decide(BOARD * board)
{
	COORDINATE coor;
	return COORDINATE();
}

COORDINATE MACHINE::getPosition(BOARD* board, int color) {//计算下一步棋子的位置
	//旗手和棋子的标志 + 1 = 棋盘的标志
	...
	for (int i = 0; i < MAX_ROW; i++)
	{
		...
		{
			machineScore[MAX_ROW][MAX_COL] = 0;	//自己分数
			peopleScore[MAX_ROW][MAX_COL] = 0;	//对手分数
		}
	}
	//打分
	for (int i = 0; i < MAX_ROW; i++)
		for (int j = 0; j < MAX_COL; j++) {
			...
			position.y = j;
			//我的分数
			score = judgeChessSituation(board->chessBoard, position, mychesspadcolor);//返回当前形势分数
			machineScore[i][j] = score;
			...
			//对手分数
			score = judgeChessSituation(board->chessBoard, position, hischesspadcolor);//返回当前形势分数
			peopleScore[i][j] = score;
		}
...
int MACHINE::luozi(BOARD* board)
{
	COORDINATE coor;
	coor = getPosition(board, 2);
...
int MACHINE::judgeChessSituation(int state[MAX_ROW][MAX_COL], COORDINATE position, int color) {
...
int MACHINE::giveScore(Situation situation) {
...
int MACHINE::getType(int state[MAX_ROW][MAX_COL], COORDINATE position, int color, int direction) {
...
void MACHINE::getChess(int chess[9], int state[MAX_ROW][MAX_COL], COORDINATE position, int color, int direction) {
...
int MACHINE::judgeType(const int chess[9]) {
	int mycolor = chess[4];
	int hiscolor;
...
COORDINATE MACHINE::maxScore() {
	COORDINATE position = { 0, 0 };

	int mymaxscore = 0;//初始化为0
	int hismaxscore = 0;

	std::vector<COORDINATE> mypositionvector;//存储最大值对应的位置向量
	std::vector<COORDINATE> hispositionvector;

	for (int i = 0; i < MAX_ROW; i++)//设置我的最大值向量
		for (int j = 0; j < MAX_COL; j++) {
			if (machineScore[i][j] == mymaxscore) {//添加位置向量数据
				position.x = i;
				position.y = j;
				mypositionvector.push_back(position);
			}
			if (machineScore[i][j]>mymaxscore) {//肯定有大于0的分数
				mymaxscore = machineScore[i][j];//替换最大值
	...
	for (int i = 0; i < MAX_ROW; i++)//设置对手的最大值向量
		for (int j = 0; j < 15; j++) {
			if (peopleScore[i][j] == hismaxscore) {
				position.x = i;
				position.y = j;
				hispositionvector.push_back(position);
			}
			if (peopleScore[i][j]>hismaxscore) {//肯定有大于0的分数
				hismaxscore = peopleScore[i][j];//替换最大值
	...
	if (mymaxscore >= hismaxscore) {//进攻 
		...
		{
			COORDINATE maxtemp = *iter;
			if (peopleScore[maxtemp.x][maxtemp.y] >= temp) {//肯定会进来
				temp = peopleScore[maxtemp.x][maxtemp.y];//替换最大值
		...
		{
			COORDINATE maxtemp = *iter;
			if (machineScore[maxtemp.x][maxtemp.y] >= temp) {//肯定会进来
				temp = machineScore[maxtemp.x][maxtemp.y];//替换最大值
				position = maxtemp;//替换位置
			}
		}
